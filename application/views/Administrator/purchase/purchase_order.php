					...
					<div class="control-group">
						<form v-on:submit.prevent="addToCart">
							...
							<div class="form-group">
								...
								<div class="col-xs-4">
									<input type="number" step="0.01" id="purchaseRate" name="purchaseRate" class="form-control" placeholder="Pur. Rate" v-model="selectedProduct.Product_Purchase_Rate" v-on:input="productTotal" autocomplete="off" />
								</div>
			...
			<div class="table-responsive">
				<table class="table table-bordered table-hover" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						...
						<tr>
							<td colspan="4"><textarea class="form-control" style="font-size:13px;margin-top:3px;" placeholder="Note" v-model="purchase.note"></textarea></td>
			...
			async onSearchSupplier(val, loading) {
				if (val.length > 2) {
					...
					await axios.post("/get_suppliers", {
						...
						.then(res => {
							let r = res.data;
							this.suppliers = r.filter(item => item.status == 'a' && item.branch_id == this.selectedBranch.branch_id)
							loading(false)
						})
				} else {
					loading(false)
			...
			async onSearchProduct(val, loading) {
				if (val.length > 2) {
					...
					await axios.post("/get_products", {
						...
						.then(res => {
							let r = res.data;
							this.products = r.filter(item => item.status == 'a' && item.is_service == 'false' && item.branch_id == this.selectedBranch.branch_id);
							loading(false)
						})
				} else {
					loading(false)
